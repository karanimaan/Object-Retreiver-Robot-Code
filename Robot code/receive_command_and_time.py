...
while True:
    ...
    request = str(request)
    print(request)
    request, start_time = request[:-1].split('&')
    print(f"phone start_time: {start_time}")
    phone_time = int(start_time)  # Convert to integer
    esp_time = int(time.time_ns() / 1000)  # Get current time in milliseconds
    print(f"esp_time: {esp_time}")
    
    latency = esp_time - phone_time
    relative_latency = latency + 946677605131
    #print(f"Latency: {latency} ms")
    #print(f'relative latency: {relative_latency/1000} s')
